...
<style lang="scss">
  .algolia-search {
    ...
    &.is-empty {
      .fs-autocomplete-suggestion__list {
        padding-bottom: 0;
      }
    }
...
<script>
  ...
  export default {
    ...
    methods: {
      initIndex() {
        const client = algoliasearch('56P9EG9SKP', 'cf4f58e7d2f61b37f3d2e448b83a2400');
        this.index = client.initIndex(`dev_fosunhis`);
      },

      querySearch(query, cb) {
        if (!query) return;
        this.index.search({ query, hitsPerPage: 8 }, (err, res) => {
          if (err) {
            console.error(err);
            return;
          }
